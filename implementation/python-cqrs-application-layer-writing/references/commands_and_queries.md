...
Command, Query и вложенные input DTO только передают данные: не добавляй в них
`__post_init__`, validators и методы проверки. Структурные ограничения проверяй
в use case до первого I/O и преобразуй нарушение в application-ошибку с
контекстом его `action`.

Если один общий input DTO, например `LimitOffsetPaginator`, используется во
многих операциях, реализуй проверку один раз в helper базового use case. В
каждой операции явно вызывай этот helper: так правило не дублируется, но состав
проверок use case остаётся видимым. Не вводи неявный глобальный validation
pipeline.
...
ID нового агрегата не включай в create-команду, если вызывающая сторона не
выбирает его по заданной внешней семантике. Use case получает новое значение
через application-порт. Клиентский ID допустим, например, для согласованного
идемпотентного `PUT`, но не добавляется по шаблону.
