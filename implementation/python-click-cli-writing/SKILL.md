...
- Считать короткое и длинное имена (`-h`/`--help`) aliases одной опции. Если
  контракт запрещает повтор, callback eager-help сначала проверяет общее число
  употреблений aliases и только затем печатает справку. Eager-поведение защищает
  help от влияния других опций, но не отменяет валидацию самой help option.
...
соответствующих flags, не дублируя пример для каждого значения. Не выводить
требуемое число или форму примеров только из множественного числа в тексте
документации. Сохранять существующий универсальный шаблон, пока нормативный
контракт явно не требует конкретных сценариев либо их количества.
...
Если CLI публикует длительность, monotonic clock является подменяемой process-
зависимостью state. Рассчитывать duration единым методом state для всех успешных
и ошибочных границ, не допускать отрицательного значения и проверять точный
результат управляемыми часами.
...
Переносить уже доступные operation metadata и переданный либо созданный
correlation ID в state до command-local validation. После появления этот контекст
не должен теряться в input, dependency, application или unexpected outcome.
...
Если CLI выбирает проекцию из общего process-конфига, считать строгую проверку
top-level envelope интеграционным инвариантом: выбор нужных секций не должен
молча отбрасывать неизвестные ключи до валидации. Реализацию и тесты выполнять по
`python-pydantic-settings-config-writing`.
...
Перед выводом об отсутствии executable прочитать README, Dockerfile и packaging-
конфигурацию проекта. Проверять фактический production launch prefix через
предусмотренный runtime, включая контейнер, если именно он формирует окружение и
`ENTRYPOINT`; не требовать отдельный console script без нормативного основания.

Не тестировать внутренности Click. Детальную организацию, fixtures,
параметризацию и coverage выполнять по `python-pytest-testing`. При добавлении
CLI-тестов отдельно проверить читаемые `ids` каждой параметризации и размещение
CLI-specific fixtures в минимально высоком общем `conftest.py`.
