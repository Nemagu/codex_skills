...
- Если контракт задаёт значения пагинации по умолчанию, объявлять query-параметры
  необязательными и подставлять defaults на presentation-границе до mapping-а.
...
Публичную ошибку реализовывать собственной transport-моделью со стабильным
машинным кодом, безопасным сообщением и структурированными деталями. Не раскрывать
внешнему клиенту нативную модель ошибок Pydantic/FastAPI как контракт сервиса.
