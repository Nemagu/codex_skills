...
3. Разделить top-level settings worker-а, общий runtime, настройки каждой задачи
...
- Единицы выражать `timedelta` либо явным суффиксом `_ms`, `_seconds`, `_bytes` и
...
- [Runtime и независимые фоновые задачи](references/background_worker_settings.md)
...
- Общий `operation_timeout`, `interval` или retry delay для независимых задач.
