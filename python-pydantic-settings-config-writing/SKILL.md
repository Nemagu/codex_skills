...
1. Извлечь точные поля, типы, обязательность, источники и приоритеты.
2. Предложить пользователю значения по умолчанию для параметров, у которых есть
...
5. Обновить минимальный и полный YAML-примеры, а также затронутую
...
- Для стандартных файловых probe headless worker-а хранить `heartbeat_file`,
...
- Существующий `*.worker.example.yaml` считать минимальным примером, а полный
...
- Параметризованно загружать минимальный и полный `*.example.yaml`
