...
отдельные минимальные порты. Не вызывай `uuid4()`/`datetime.now()` в use case и не
размещай `next_id()` в repository-порте. Передавай ID в domain-фабрику готовым VO.
...
Use case, domain, UoW и пробрасывающий адаптер не логируют ошибку. Итог один раз
логируется на внешней границе: HTTP error handler, consumer или worker iteration.
Адаптер логирует только обработанную локальную попытку, которая не выходит наружу.
