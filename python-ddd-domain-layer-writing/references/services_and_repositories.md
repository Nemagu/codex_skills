...
Domain service реализует заданное бизнес-правило, которое не принадлежит одному
...
| Требует специализированного поиска как части правила | **Domain service** с узким read-интерфейсом |
...
Если необходимые объекты можно загрузить заранее без воспроизведения бизнес-
алгоритма, передавай их в сервис готовыми. Application задаёт порядок загрузки,
но само правило остаётся в domain.

### Общий контракт

Любой заданный domain-сервис подчиняется одному набору правил независимо от назначения.

**1. Когда создавать сервис.** Создавай только сервисы, явно заданные входным
контрактом. Не переноси в них поведение агрегата и orchestration вызывающего слоя.
...
- **С I/O** → конструктор принимает только заданные узкие read-интерфейсы из
  `domain/<aggregate>/repository.py`; сервис не загружает несколько объектов ради
  orchestration и не имеет изменяемого состояния.
...
- Создание и сохранение агрегатов; создание выполняется вызывающим слоем через фабрику.
- Генерация ID.
- Вызов фабрики.
