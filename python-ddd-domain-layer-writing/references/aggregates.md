...
Фабрика — единственная точка создания и восстановления агрегата из готовых VO:

- Зафиксировать только заданные начальные значения.
...
- Прямой вызов конструктора агрегата за пределами фабрики запрещён.
...
- Принимает переданный вызывающим слоем ID как VO.
- Не принимает поля, для которых задано однозначное начальное значение.

**Реализация:**
- Передаёт только заданные начальные `state`, `version` и бизнес-значения.
- Не добавляет типовые состояния и значения по умолчанию.
...
ID для нового агрегата фабрика не генерирует и не получает через зависимость.
Вызывающий слой передаёт готовый ID-VO в `new()`.
...
4. Выполни все заданные проверки состояния, инвариантов и семантики повтора.
5. После проверок выполни мутацию.
6. В конце увеличь версию ровно один раз.
...
❌ **Проверка состояния, отсутствующая во входном контракте.**
...
❌ **Получение ID в фабрике.** Передай готовый ID-VO из вызывающего слоя.
