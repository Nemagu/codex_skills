...
`PublishDTO` в публичной сигнатуре является application `TypeAlias` закрытого
объединения конкретных DTO. Реестр маршрутов не заменяет этот alias и не требует
application enum типов ресурсов. Покрой каждый член alias отдельным случаем
mapping и защитным исходом для неизвестного runtime-типа.
