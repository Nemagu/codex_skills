...
Для нового сервиса запуск оформлять через import-safe `entrypoints/api.py`:
`main()` загружает settings, выполняет preflight и вызывает
`APIWorker(settings).run()`. Не создавать worker и не выполнять I/O на уровне
модуля.
